<br>
<!-- Footer Start -->
<footer class="footer">
    <div class="container">
        <div class="clearfix">
            <div class="footer-content">
                <h4 class="copyright">COPYRIGHT &copy; <?php echo date('Y'); ?> <?php echo strtoupper(SITE_NAME); ?> -
                    ALL RIGHTS RESERVED.</h4>
            </div>
        </div>
    </div>
</footer>
<!-- Footer End -->
</div>
<!--JS -->
<script type="text/javascript" src="<?php echo ASSETS_URL; ?>js/jquery.min.js"></script>
<script type="text/javascript" src="<?php echo ASSETS_URL; ?>js/popper.js"></script>
<script type="text/javascript" src="<?php echo ASSETS_URL; ?>js/bootstrap.min.js"></script>
</body>

</html>